...
```{r}
cols_meta <- read.csv("../../meta/collections_texts.csv") %>% select(-X)
```
...
```{r}
write.csv(cols_texts %>% select(-path_raw, -path_acc), 
...
cols_corpus <- cols_meta_united %>% 
  left_join(cols_texts %>% select(-path_raw, -path_acc),
            by = "text_id")
...
save(cols_meta, cols_texts, cols_meta_united, cols_corpus,
...
-   cols_texts - dataframe with texts only;
