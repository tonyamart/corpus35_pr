...
```{r}
lf <- list.files("../../data/collections/",
                 pattern = ".txt", 
                 full.names = TRUE)
...
dat_cln <- dat_meta %>% 
  ...
  mutate(path_text = paste0("../../data/collections/texts//", text_id, ".txt" )) %>% 
...
write.csv(dat_cln %>% select(-text, -text_cln), 
          file = "../../meta/collections_texts.csv") 
```
...
dat_meta_short <- dat_meta %>% 
  select(-text) %>% 
  mutate(id = str_replace_all(path, "../../data/collections//(\\d+)_\\d+\\.txt",
                              "\\1"),
         id = as.numeric(id)) %>% 
...
Read metadata for collections

```{r}
cols_meta <- read.csv("../../meta/collections_meta.csv") %>% select(-X)
glimpse(cols_meta)
...
cols_dig <- cols_meta %>% 
  # add one missed collection
  add_row(id = 1027,
          poetry_prose = "v",
          book_type = "col",
          genre = "lyr",
          author_sign = "Кожухов Н.",
          author = "Кожухов Н.C",
          author_full_name = "Кожухов Николай Степанович",
          title = "Стихотворения Н. Кожухова",
          city = "М.",
          publisher = "Ун. тип",
          year = 1835, 
          pages = 24,
          size_fold = 8, 
          size_cm = 20,
          group = "Сборники стихотворений",
          group_abbr = "col_lyr",
          group_eng = "Lyric collections") %>% 
  
  inner_join(dat_meta_short, by = "id") %>% # filter only books which were digitized
  
  rbind(fem_cols) # add fem collections
```
...
```{r}
# write.csv(cols_dig, "../../meta/collections_digitised_meta.csv")
```

### Attach female texts from separate folder

```{r}
f <- list.files(path = "../../data/collections/fem_cols/",
                pattern = ".txt",
                full.names = T,
                recursive = T)
...
fem_texts <- tibble(path = f,
  ...
  mutate(id = str_extract(path, "C_\\d+"),
         id = str_remove_all(id, "C_")) 

str(fem_texts)

# find old meta & attach first lines & titles
# then attach all to main corpus
# store as RDs & meta as csv-meta
```
