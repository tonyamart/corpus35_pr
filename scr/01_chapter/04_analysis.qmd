...
# ggsave("plots/Fig_1-4-1.png", plot = last_plot(), dpi = 300,
#         width = 11, height = 7,
#         bg = "white")
```
...
perc_digit <- pb_full %>% 
    ...
    ggplot(aes(x = year, y = n, fill = as.factor(dig))) + 
        ...
        facet_wrap(~group, 
                   scales = "free_x",
                   ncol = 4) + 
        scale_fill_manual(values = c(c("grey85",
                                     "grey20"))) + 
        scale_x_continuous(breaks = seq(1830, 1850, 5)) +
        labs(x = "Год",
             y = "Процент книг",
...
ggsave("plots/bw/Fig_1-4-1.png", plot = last_plot(), dpi = 300,
        width = 11, height = 7,
        bg = "white")
```
...
```{r}
# test how many books contain prose (except for almanacks)
pb %>%
    filter(poetry_prose == "pr" & book_type != "alm")
```

#### Plot 1.4.2.a. Number of books by category
...
# ggsave(file = "plots/Fig_1-4-2-a.png", plot = last_plot(), dpi = 300,
#      width = 10, height = 8, bg = "white")
```

#### Plot 1.4.2.b. Boxplots w/ n of pages and size in cm
...
#ggsave("plots/Fig_1-4-2-b.png", plot = last_plot(), dpi = 300, bg = "white",
#       width = 12, height = 8)
```
...
pb %>% 
    ...
    ggplot(aes(x = year, y = n,# color = group, 
        ...
        #facet_wrap(~group_rus, scales = "free_x") +
        geom_col(position = "dodge") + 
        scale_fill_manual(values = c("grey10", "grey80", "grey40")) + 
        scale_x_continuous(breaks = seq(1835,1840,1)) +
        theme(
            legend.position = "bottom",
            axis.title = element_text(size = 14),
...
# ggsave(file = "plots/bw/Fig_1-4-2-a.png", plot = last_plot(), dpi = 300,
#      width = 10, height = 8, bg = "white")
```
...
p1 <- pb %>% 
    ...
    ggplot(aes(x = year, y = pages, group = year, 
        ...
        scale_x_continuous(breaks = seq(1835, 1840, 1)) +
        scale_fill_manual(values = c("grey35", "grey90", "grey65")) + 
        theme(
            legend.position = "None",
            axis.text = element_text(size = 12),
            axis.text.x = element_blank(), # removed years from x axis for the upper plot
...
ggsave("plots/bw/Fig_1-4-2-b.png", plot = last_plot(), dpi = 300, bg = "white",
       width = 12, height = 8)
```
...
# ggsave("plots/Fig_1-4-3.png", plot = last_plot(), dpi = 300,
#        height = 10, width = 8,
#        bg = "white")
```
...
publ_plot_data %>% 
    ggplot(aes(x = as.factor(year), y = cum_sum, #color = typo_group, 
               group = typo)) + 
        geom_line(alpha = 0.6, linewidth = 1, colour = "grey10") + 
        gghighlight(use_direct_label = FALSE,
...
# ggsave("plots/bw/Fig_1-4-3.png", plot = last_plot(), dpi = 300,
#         height = 10, width = 8,
#         bg = "white")
```
