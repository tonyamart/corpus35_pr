...
Old collections meta:

```{r}
old_pb <- read.delim("../../meta/old/meta_collections.tsv", sep = "\t") %>% 
  select(ID, A_ID, Author, Year, Title)
glimpse(old_pb)
...
Load old authors metadata

```{r}
authors <- read.delim("../../meta/old/authors.tsv", sep = "\t") %>% 
  filter(Author_Initials != "")
glimpse(authors)
```

Join PB and authors data, see who's left

```{r}
t <- pb_authors %>% 
  left_join(authors, by = "A_ID") %>% 
  select(id, COL_ID, A_ID, author, Author_Initials) %>% distinct
...
```{r}
authors
authors[which(authors$Author_Initials == "Баратынский Е.А."), 1:2]
```

```{r}
x <- as.data.frame(tibble(author = sort(unique(pb$author))))
x

write.csv(x = x, file = "../../meta/authors.csv")
```
