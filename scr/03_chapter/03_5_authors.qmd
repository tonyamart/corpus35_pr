...
metrical_mtrx <- corpus1835 %>% 
  ...
  # leave only clearly detected & frequent meters
  mutate(formula = paste0(meter, "_", feet)) %>% 
  filter(formula %in% c("Iamb_4", "Iamb_v", "Trochee_4", "Iamb_6", "Iamb_5", 
                        "Amphibrach_4", "Iamb_3", "Anapest_other", 
                        "Amphibrach_43", "Iamb_43", "Amphibrach_3", "Anapest_2", 
                        "Trochee_3", "Amphibrach_2", "Dactyl_3", "Trochee_43", 
                        "Amphibrach_other", "Dactyl_6")) %>% 
  ...
  group_by(author_text) %>% 
  sample_n(900) %>% # sample 900 lines from each author
  #ungroup() %>% 
  
  # create two samples of 500 lines for each author
  mutate(sample_id = ceiling(1:900),
         sample_id = floor(sample_id/450)+1,
...
length(colnames(metrical_mtrx))-1
```
...
Create a topic model to see the distribution of topics in the two classes of authors
