...
network <- function(network, meter_value, palette_v) { 
  ggraph(network, layout = "stress") +
    geom_edge_fan(aes(color = meter_r,
...
# write.csv(topic_labels, file = "../../data/ch4/topic_labels.csv")
```
...
ggsave(filename = "plots/Fig_4-2-1.png", plot = last_plot(),
...
edgelist <- edgelist %>% 
  mutate(meter_r = str_replace(meter, "iamb", "ямб"),
         meter_r = str_replace(meter_r, "trochee", "хорей"),
         meter_r = str_replace(meter_r, "free", "вольн."),
         meter_r = str_replace(meter_r, "amphibrach", "Амф")
  ) 
```
...
### Fig_4-2-2-B - All without Iamb-4
...
```{r, eval=FALSE}
ggsave(filename = "plots/Fig_4-2-2-b.png", plot = last_plot(), 
...
# ggsave(filename = "plots/bw/Fig_4-2-2-b.png", plot = last_plot(), 
#       width = 9, height = 7, dpi = 300, bg = "white")
```
...
Iamb-4 networks with densities

```{r}
# create labels df
labels <- density_table %>% 
  filter(meter == "iamb_4") %>% 
  mutate(slice_d = paste0(slice, "\n d = ", density))

# transform to named vector
facet_labels <- setNames(as.character(labels$slice_d), labels$slice)
...
iamb_4 + facet_wrap(~slice, 
                 ...
                 drop=T,
                 ncol = 5, 
                 labeller = as_labeller(facet_labels)) + # labels attached
    theme(legend.position = "None",
         strip.text = element_text(size = 16),
         legend.text = element_text(size = 14), 
         legend.title = element_text(size = 16))
...
```{r, eval=FALSE}
ggsave(filename = "plots/Fig_4-2-2-a.png", plot = last_plot(),
       width = 9, height = 7, dpi = 300, bg = "white")
```
...
```{r, eval=FALSE}
# create network
iamb_4 <- network(net, c("iamb_4"), "gray35")
...
ggsave(filename = "plots/bw/Fig_4-2-2-a.png", plot = last_plot(),
       width = 9, height = 7, dpi = 300, bg = "white")
```
...
This is the part there number of the same edges in meters pairwise calculated in comparison with a random baseline. The code is based on the previous work: (Martynenko, Sela 2023).
...
### Fig_4_2_4 - edges stability

```{r}
head(all_edges)

unique(all_edges$meter)

transl_meters <- tibble(
  meter = c("Iamb_4", "Iamb_6", "Trochee_4"),
  meter_r = c("4-ст. ямб", "6-ст. ямб", "4-ст. хорей")
)

all_edges %>% 
  left_join(transl_meters, by = "meter") %>% 
  mutate(corpus_r = ifelse(corpus == "M", "Корпус-1835", "НКРЯ")) %>% 
  ggplot(aes(x = meter_r, y = Freq, , fill = corpus_r)) + 
  geom_boxplot(alpha = 0.7) + 
  labs(y = "Стабильность связи",
       fill = "Корпус") + 
  scale_fill_manual(values = c(plt[4], plt[2])) + 
...
```{r, eval=FALSE}
ggsave(filename = "plots/Fig_4-2-4.png", plot = last_plot(),
...
ggsave(filename = "plots/bw/Fig_4-2-4.png", plot = last_plot(),
       width = 7, height = 5, dpi = 300, bg = "white")
```
...
p_i4 <- network_labels(network = net_comp, 
                       meter_value = "iamb_4", 
                       palette_v = c("grey25", "grey80")) + 
  labs(title = "Корпус-1835            vs              НКРЯ",
       subtitle = "4-ст. ямб") + 
...
p_i6 <- network_labels(network = net_comp, 
                       meter_value = "iamb_6", 
                       palette_v = c("grey25", "grey80")) + 
  labs(subtitle = "6-ст. ямб") + 
  theme(plot.subtitle = element_text(size = 22, face = "bold"))

p_t4 <- network_labels(network = net_comp, 
                       meter_value = "trochee_4", 
                       palette_v = c("grey25", "grey80")) + 
  labs(subtitle = "4-ст. хорей") + 
  theme(plot.subtitle = element_text(size = 22, face = "bold"))

plot_grid(p_i4, p_i6, p_t4, 
          labels = NULL,
          ncol=1)

ggsave(file = "plots/bw/Fig_4-2-5.png", plot = last_plot(),
      dpi = 300, width = 8, height = 16, bg = "white")
```
