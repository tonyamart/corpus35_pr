...
length(unique(rhymes$rhyme_pair)) # number of unique rhymes - 59 584
```
...
```{r}
# quick test on the first 10 words
...
```{r}
# real top of the words on these endings in RNC
...
rnc_rhymer %>% 
  ggplot(aes(x = rank, y = n, fill = group)) + geom_col() + theme_minimal() #+ 
