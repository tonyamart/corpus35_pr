...
# cv_rhyme <- res %>% 
#   #filter(rank < 2001) %>% 
#   ggplot(aes(x = rank, y = CV)) + 
#   geom_point(shape = 1, colour = met.brewer("Veronese")[6], alpha = 0.6) + 
#   geom_vline(xintercept = 300, lty = 3) + 
#   geom_smooth(colour = met.brewer("Veronese")[3]) + 
#   labs(x = "", y = "Коэффициент вариации") + 
...
cv_plt <- rbind(cv_rhymes, cv_all) %>% 
  ...
  geom_vline(xintercept = 300, lty = 3) + 
  geom_smooth(color = met.brewer("Veronese")[3]) + 
