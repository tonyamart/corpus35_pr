...
```{r}
rhymes <- read_csv("../../data/corpus1835/sql_db/rhyme_pairs.csv") %>% 
  # remove Kulman texts
  filter(!str_detect(poem_id, "C_264"))
glimpse(rhymes)
```
...
Add ending stress annotation ------------ move this to 05_2 !
...
tr_rhymes %>% 
  ...
  filter(from_closure == "dactylic" & to_closure == "dactylic") %>% 
  select(id, from_upos, to_upos) %>% 
  pivot_longer(!id) %>% 
  count(value, sort = T) %>% 
...
iamb_masc %>% 
  ...
  mutate(rank = row_number()) %>% 
  filter(rank < 1001) %>% 
  ggplot(aes(x = rank, y = n)) + geom_col(fill = met.brewer("Veronese")[4],
                                          alpha = 0.6) + 
  geom_line(linewidth = 0.6, color = met.brewer("Veronese")[7]) + 
  
  labs(x = "Ранг", y = "Частота")
```
...
iamb_masc %>% 
  ...
  geom_line(linewidth = 0.3, color = met.brewer("Veronese")[7]) + 
  geom_col(alpha = 0.3, fill = met.brewer("Veronese")[4]) +
  labs(x = "Ранг окончания по частотности", 
       y = "Количество комбинаций \nчастей речи внутри окончания")
```

```{r}

im_upos <- iamb_masc %>% 
  filter(!str_detect(upos_pair, "NA") #& !str_detect(ending, "дней")
         ) %>% 
  group_by(ending) %>% 
  count(upos_pair, sort = F) %>% 
  count(ending, sort = T) %>% 
  left_join(ranks, by = "ending") %>% 
  filter(rank > 21 & rank < 51) %>% 
  mutate(label = paste0(rank, ending),
         group = ifelse(n < 5, "Менее вариатиавная", "Более вариативная"))

im_upos %>% 
  ggplot(aes(x = rank, y = n, fill = group)) + 
  geom_col(alpha = 0.8) + 
  coord_flip() + scale_x_reverse(breaks = im_upos$rank,
                                 labels = im_upos$label) + 
  scale_fill_manual(values = c(met.brewer("Veronese")[4],
                               met.brewer("Veronese")[3])) + 
  labs(x = "Ранг окончания по частотности",
       y = "Количество комбинаций частей речи",
       fill = "") + 
  theme(legend.position = "bottom", 
        text = element_text(size = 18)) + 
  ylim(c(0, 25))

ggsave("plots/pos_variation.png", plot = last_plot(), 
...
im_rank_feat <- iamb_masc %>% 
  ...
  filter(rank < 41) %>% 
  mutate(label = paste0(rank, " ", ending),
         group = ifelse(n < 25, "less variation", "more variation")) 
