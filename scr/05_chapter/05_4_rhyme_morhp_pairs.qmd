...
What is are the probabilities to have POS_x of n_syl after a POS_j of nj_syl ?

The most freq word type in masculine rhymes: NOUN_2, then NOUN_1 and VERB_2
...
words_1 <- words$from_pos_syl
words_2 <- words$to_pos_syl

x <- tibble(pos_syl = c(words_1, words_2)) %>% 
  count(pos_syl, sort = T) %>% head(10)
x
top_pos <- x$pos_syl

rm(words, words_1, words_2)
```

top-freq pos possibilities
...
top_pos_total <- masc_pairs %>% 
  ...
  # replace NOUN_2 by other combination for counting
  # mutate(from_pos_syl = ifelse(from_pos_syl == "NOUN_2", "!_NOUN_2", from_pos_syl),
  #        to_pos_syl = ifelse(to_pos_syl == "NOUN_2", "!_NOUN_2", to_pos_syl)) %>% 
  #rowwise() %>% 
  # mutate(pos_pair = paste0(from_pos_syl, " -- ", to_pos_syl)) %>%
  #ungroup() %>% 
  select(from_pos_syl, to_pos_syl) %>% 
  filter(from_pos_syl %in% top_pos) %>% 
  
  # total number of this pos_syl combination as the first word in rhyme
  count(from_pos_syl, sort = T) %>% 
  rename(total = n)

top_pos_total
```

Count probs of different pos following the first one

```{r}
top_probs <- masc_pairs %>% 
  mutate(# calculate number of syllables in each word
         from_n_syl = nchar(from_sp),
         to_n_syl = nchar(to_sp),
  ...
  select(from_pos_syl, to_pos_syl) %>% 
  filter(from_pos_syl %in% top_pos) %>% 
  group_by(from_pos_syl) %>% 
  count(to_pos_syl, sort = T) %>% 
  left_join(top_pos_total, by = "from_pos_syl") %>% 
  mutate(prob = n/total)
...
```{r}
node_size <- tibble(name = c(top_probs$from_pos_syl, top_probs$to_pos_syl)) %>% 
  count(name)
...
top_probs %>% 
  ...
  as_tbl_graph() %>% 
  ggraph() + 
  geom_edge_fan(aes(width = prob, color = prob)) + 
  geom_node_point() + 
  geom_node_label(aes(label = name), repel = TRUE)

# top_probs %>% 
#   filter(from_pos_syl == "NOUN_3") %>% 
#   mutate(prob = round(prob, 3)) %>% 
#   as_tbl_graph() %>% 
#   activate(nodes) %>% 
#   left_join(node_size, by = "name") %>% 
#   #mutate(label = ifelse(n > 1, name, NA)) %>% 
#   ggraph(layout = 'tree') + 
#   geom_edge_diagonal(
#     aes(colour = prob,
#         width = prob)
...
noun_2 <- masc_pairs %>% 
  ...
  ungroup() %>% 
  select(pos_pair, meter, feet) %>% 
  separate(pos_pair, into = c("pos_1", "pos_2"), sep = " -- ") %>% 
  filter(pos_1 == "!_NOUN_2" & meter != "Other" & feet != "other")
...
noun_2 %>% 
  group_by(pos_1) %>% 
  count(pos_2, sort = T) %>% 
  mutate(prob = round((n / nrow(noun_2)) *100 , 1))
```
...
```{r}
x <- noun_2 %>% sample_n(10)
x


t <- x %>% 
  mutate(#meter = paste0(meter, "_", feet),
         clausula = c(rep("masc", 9), "fem")) %>% 
  separate(pos_2, into = c("pos", "n_syl"), sep = "_") %>% 
  select(-pos_1, -feet)
 

t <- tibble(
  meter = c("iamb", "iamb", "iamb", "trochee", "trochee", "trochee"),
  n_syl = c(2,2,3, 3,3,3),
  pos = c("NOUN", "VERB", "PRON", "PRON", "ADJ", "PRON"),
  n_feet = c(2,2,3,3,3,3)
)
 
t

trdata <- NULL
trdata$meter <- as.factor(t$meter)
trdata$n_syl <- as.numeric(t$n_syl)
trdata$n_feet <- as.numeric(t$n_feet)
trdata$pos <- as.factor(t$pos)


trdata

rfit <- rpart(pos ~ n_syl, data = trdata, method = "class")

rpart.plot(rfit, box.palette = "Blues", roundint = FALSE)
?rpart.plot
```
