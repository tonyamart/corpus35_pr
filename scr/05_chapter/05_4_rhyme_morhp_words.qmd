...
all_words %>% 
  ...
  filter(corpus == "P" & closure != "other") %>% 
  count(pos) %>% 
  mutate(perc_total = round(
    n / nrow(
      all_words %>% mutate(corpus = str_extract(text_id, "^\\w")) %>% 
          filter(corpus == "P" & closure != "other")
  ) * 100, 
    2))
...
```{r}
total_corpus <- all_words %>% 
  filter(meter == "Iamb" & closure %in% c("masc", "fem")) %>% 
  mutate(corpus = str_extract(text_id, "^\\w")) %>%
  count(corpus, closure) %>% 
  rename(total_closure = n) %>% 
  mutate(corpus_closure = paste0(corpus, "_", closure))
...
all_words %>% 
  mutate(corpus = str_extract(text_id, "^\\w"),
         corpus_closure = paste0(corpus, "_", closure)) %>%
  #filter(corpus == "P") %>% 
  filter(meter == "Iamb" & closure %in% c("masc", "fem")) %>% 
  group_by(corpus_closure) %>% 
  count(pos) %>% 
  ungroup() %>% 
  #left_join(total_meter, by = "meter") %>% 
  ...
  # percentage of all words in iamb
  #mutate(perc_all_iambic = round((n / total) * 100, 2 )) # %>% 
  left_join(total_corpus, by = "corpus_closure") %>% 
  # 
  # # percentage of word of particular clausula type
  mutate(perc_iamb_clos = round((n / total_closure) * 100, 2 )) %>% 
  select(-corpus_closure, -total_closure) %>% 
  filter(closure == "masc")
```
...
all_words %>% 
  mutate(corpus = str_extract(text_id, "^\\w"),
         corpus_closure = paste0(corpus, "_", closure)) %>% 
  filter(meter == "Iamb" & closure %in% c("fem", "masc")) %>% 
  group_by(corpus_closure) %>% 
  count(pos_group) %>% 
  ungroup() %>% 
  #left_join(total_meter, by = "meter") %>% 
